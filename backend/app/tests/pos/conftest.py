...
Fixtures:
  ...
  pos_client      — app dependency override qilingan AsyncClient
  ...
  admin_user, store_user, accountant_user, agent_user, courier_user
"""
...
async def default_enterprise(db_session: AsyncSession) -> Enterprise:
    """MT1: Default test korxonasi — POS moduli yoqilgan."""
    ent = Enterprise(
...
def make_product(db_session: AsyncSession, default_enterprise: Enterprise):
    """Factory: Product va narx bilan yaratadi."""
    ...
    async def _factory(
        ...
        if price is not None and segment_id is not None:
            pp = ProductPrice(
                product_id=prod_id,
...
@pytest.fixture
async def pos_client(db_session: AsyncSession, fake_redis):
